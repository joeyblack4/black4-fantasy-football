...
// scripts/agent/channel-agent-helper.src.mjs
...
function weierstrass(params2, extraOpts = {}) {
  const validated = createCurveFields("weierstrass", params2, extraOpts);
...
function challenge(...args2) {
  return Pointk1.Fn.create(num(taggedHash("BIP0340/challenge", ...args2)));
}
function schnorrGetPublicKey(secretKey2) {
  return schnorrGetExtPubKey(secretKey2).bytes;
}
function schnorrSign(message, secretKey2, auxRand = randomBytes(32)) {
  const { Fn, BASE } = Pointk1;
  const m = abytes2(message, void 0, "message");
  const { bytes: px, scalar: d } = schnorrGetExtPubKey(secretKey2);
...
var BECH32_GENERATORS = [996825010, 642813549, 513874426, 1027748829, 705979059];
...
function decodeNsec(input) {
  ...
  if (!lowered.startsWith("nsec1") || separator < 1) return null;
  const expanded = [];
  ...
  const values = [];
  for (const char of lowered.slice(separator + 1)) {
    const value = BECH32_CHARSET.indexOf(char);
    if (value === -1) return null;
    values.push(value);
  }
  if (bech32Polymod([...expanded, ...values]) !== 1) return null;
  ...
  if (bits >= 5 || accumulator << 8 - bits & 255) return null;
...
function nip98Header(secretKey2, url2, method, body2) {
  const event = {
    pubkey: bytesToHex3(schnorr.getPublicKey(secretKey2)),
    created_at: Math.floor(Date.now() / 1e3),
    kind: 27235,
    tags: [
      ["u", url2],
      ["method", method],
      ["payload", createHash("sha256").update(body2, "utf8").digest("hex")],
      ["nonce", randomBytes3(8).toString("hex")]
    ],
    content: ""
  };
  const id = createHash("sha256").update(JSON.stringify([0, event.pubkey, event.created_at, event.kind, event.tags, event.content]), "utf8").digest("hex");
  const idBytes = new Uint8Array(32);
  for (let index = 0; index < 32; index += 1) {
    idBytes[index] = Number.parseInt(id.slice(index * 2, index * 2 + 2), 16);
  }
  const sig = bytesToHex3(schnorr.sign(idBytes, secretKey2));
  return `Nostr ${Buffer.from(JSON.stringify({ ...event, id, sig }), "utf8").toString("base64")}`;
}
var ALIASES = {
  "ahrefs.usage": { catalog: "catalog-ahrefs", connector: "ahrefs", method: "GET", endpoint: "/subscription-info/limits-and-usage" },
  "ahrefs.keywords": { catalog: "catalog-ahrefs", connector: "ahrefs", method: "GET", endpoint: "/site-explorer/organic-keywords" },
  "ahrefs.pages": { catalog: "catalog-ahrefs", connector: "ahrefs", method: "GET", endpoint: "/site-explorer/top-pages" },
  "ahrefs.competitors": { catalog: "catalog-ahrefs", connector: "ahrefs", method: "GET", endpoint: "/site-explorer/organic-competitors" },
  "gmail.messages": { catalog: "catalog-google-workspace", connector: "google-workspace", method: "GET", endpoint: "/gmail/v1/users/me/messages" },
  "drive.files": { catalog: "catalog-google-workspace", connector: "google-workspace", method: "GET", endpoint: "/drive/v3/files" },
  "calendar.events": { catalog: "catalog-google-workspace", connector: "google-workspace", method: "GET", endpoint: "/calendar/v3/calendars/primary/events" }
};
function parseArgs(argv) {
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    args2[flag.slice(2)] = argv[index + 1];
    index += 1;
  }
  return args2;
}
function fail(message) {
  console.error(`channel-agent-helper: ${message}`);
  process.exit(1);
}
var positional = process.argv.slice(2).filter((token) => !token.startsWith("--"));
var args = parseArgs(process.argv.slice(2));
var baseUrl = (args.url || process.env.PLATFORM_CONTROL_URL || "").trim().replace(/\/+$/, "");
if (!baseUrl) fail("pass --url https://<platform-control-host> (or set PLATFORM_CONTROL_URL)");
var communityId = args.community || fail("--community is required");
var channelId = args.channel || fail("--channel is required");
async function questionRoute(route, extraBody) {
  const secretKey2 = decodeSecretKey(process.env.BUZZ_PRIVATE_KEY || process.env.NOSTR_PRIVATE_KEY || "");
  if (!secretKey2) fail("no agent key: BUZZ_PRIVATE_KEY (or NOSTR_PRIVATE_KEY) must hold an nsec or 64-hex key");
  const routeUrl = `${baseUrl}/api/v1/channel-capability/${route}`;
  const routeBody = JSON.stringify({ communityId, channelId, ...extraBody });
  let response2;
  try {
    response2 = await fetch(routeUrl, {
      method: "POST",
      headers: {
        authorization: nip98Header(secretKey2, routeUrl, "POST", routeBody),
        "content-type": "application/json"
      },
      body: routeBody
    });
  } catch (error) {
    fail(`could not reach ${routeUrl}: ${error && error.cause ? error.cause.message || error.cause : error.message}`);
  }
...
if (positional[0] === "warehouse") {
  const WAREHOUSE_OPS = "schema, describe, query, freshness, changes, propose-view, propose-metric";
  const op = positional[1] || fail(`warehouse needs one of: ${WAREHOUSE_OPS}`);
  if (op === "schema") await questionRoute("warehouse", { operation: "warehouse.schema" });
  if (op === "freshness") await questionRoute("warehouse", { operation: "warehouse.freshness" });
  if (op === "changes") {
    const table = positional[2] || args.table || fail("warehouse changes needs a table name");
    const since = args.since || fail("warehouse changes needs --since <iso-timestamp>");
    const limit = args.limit ? Number(args.limit) : void 0;
    await questionRoute("warehouse", { operation: "warehouse.changes", table, since, ...limit ? { limit } : {}, ...args.column ? { column: args.column } : {} });
  }
  if (op === "describe") {
    const table = positional[2] || args.table || fail("warehouse describe needs a table name");
    await questionRoute("warehouse", { operation: "warehouse.describe", table, ...args.layer ? { layer: args.layer } : {} });
  }
  if (op === "query") {
    const sql = args.sql || fail("warehouse query needs --sql '<one read-only SELECT>'");
    const limit = args.limit ? Number(args.limit) : void 0;
    await questionRoute("warehouse", { operation: "warehouse.query", sql, ...limit ? { limit } : {} });
  }
  if (op === "propose-view" || op === "propose-metric") {
    const name = args.name || fail(`warehouse ${op} needs --name <definition-name>`);
    const sql = args.sql || fail(`warehouse ${op} needs --sql '<one read-only SELECT>'`);
    if (op === "propose-view") {
      await questionRoute("warehouse", {
        operation: "warehouse.propose_view",
        name,
        sql,
    ...
    if (args.direction && args.direction !== "up" && args.direction !== "down") {
      fail("warehouse propose-metric --direction must be 'up' or 'down'");
    }
    const aliases = args.aliases ? args.aliases.split(",").map((alias) => alias.trim()).filter(Boolean) : void 0;
    await questionRoute("warehouse", {
      operation: "warehouse.propose_metric",
      name,
      sql,
      ...args.description ? { description: args.description } : {},
  ...
  fail(`unknown warehouse operation '${op}' (expected one of: ${WAREHOUSE_OPS})`);
}
if (positional[0] === "discover") {
  await questionRoute("discover", {});
}
if (positional[0] === "social") {
  const operation2 = positional[1] || fail("social needs list, get, or upsert");
  if (operation2 === "list") await questionRoute("social-content", { operation: operation2, ...args.limit ? { limit: Number(args.limit) } : {} });
  if (operation2 === "get") await questionRoute("social-content", { operation: operation2, socialContentId: args["content-id"] || fail("social get needs --content-id") });
  if (operation2 === "upsert") {
    let record;
    try {
      record = JSON.parse(args.record || fail("social upsert needs --record '<json>'"));
    } catch {
      fail("--record must be valid JSON");
    }
    await questionRoute("social-content", { operation: operation2, record });
  }
  fail(`unknown social operation '${operation2}' (expected list, get, or upsert)`);
}
if (positional[0] === "context") {
  await questionRoute("context", {});
}
if (positional[0] === "remember") {
  const kind = args.kind || fail("remember needs --kind fact|decision|constraint|contact|lesson");
  const content = args.content || fail('remember needs --content "..."');
  await questionRoute("remember", {
    kind,
    content,
    ...args.confidence ? { confidence: Number(args.confidence) } : {},
    ...args.sensitivity ? { sensitivity: args.sensitivity } : {},
    ...args.thread ? { threadId: args.thread } : {}
  });
}
if (positional[0] === "mcp") {
  const connector = args.connector || fail("mcp needs --connector <name>");
  const catalogId = args.catalog || fail("mcp needs --catalog <catalog-id>");
  const secretKey2 = decodeSecretKey(process.env.BUZZ_PRIVATE_KEY || process.env.NOSTR_PRIVATE_KEY || "");
  if (!secretKey2) fail("no agent key: BUZZ_PRIVATE_KEY (or NOSTR_PRIVATE_KEY) must hold an nsec or 64-hex key");
  const routeUrl = `${baseUrl}/api/v1/channel-capability/mcp`;
  let sessionId;
  let buffered = "";
...
`);
          ...
          console.error(`channel-agent-helper: MCP notification failed (${response2.status}): ${payload?.error?.message || "unknown error"}`);
        }
        return;
      }
      if (typeof payload.sessionId === "string" && payload.sessionId) sessionId = payload.sessionId;
      if (payload.message !== void 0) process.stdout.write(`${JSON.stringify(payload.message)}
`);
    } catch (error) {
      if (message && Object.prototype.hasOwnProperty.call(message, "id")) {
        process.stdout.write(`${JSON.stringify({
          jsonrpc: "2.0",
          id: message.id,
          error: { code: -32e3, message: `Platform Control is unreachable: ${error?.message || error}` }
        })}
`);
      }
    }
...
var operation;
if (args.action) {
  operation = ALIASES[args.action] || fail(`unknown alias '${args.action}' (aliases: ${Object.keys(ALIASES).join(", ")}) \u2014 or call directly with --connector --method --endpoint`);
} else {
  const connector = args.connector || fail("pass --action <alias> or --connector <name> with --tool or --endpoint");
  operation = args.tool ? { catalog: args.catalog || `catalog-${connector}`, connector, kind: "tool", tool: args.tool } : {
    catalog: args.catalog || `catalog-${connector}`,
    connector,
    method: (args.method || "GET").toUpperCase(),
    endpoint: args.endpoint || fail("--endpoint is required")
  };
}
var params;
if (args.params) {
...
if (args.arguments) {
  try {
    toolArguments = JSON.parse(args.arguments);
  } catch {
    fail("--arguments must be valid JSON");
  }
}
var secretKey = decodeSecretKey(process.env.BUZZ_PRIVATE_KEY || process.env.NOSTR_PRIVATE_KEY || "");
if (!secretKey) fail("no agent key: BUZZ_PRIVATE_KEY (or NOSTR_PRIVATE_KEY) must hold an nsec or 64-hex key");
var url = `${baseUrl}/api/v1/channel-capability`;
var body = JSON.stringify({
  ...
  payload: operation.kind === "tool" ? { kind: "tool", connector: operation.connector, tool: operation.tool, arguments: toolArguments || {} } : {
    ...
    ...args["idempotency-key"] ? { idempotencyKey: args["idempotency-key"] } : {}
  }
});
var response;
try {
  response = await fetch(url, {
    method: "POST",
    headers: {
      authorization: nip98Header(secretKey, url, "POST", body),
      "content-type": "application/json"
    },
    body
  });
} catch (error) {
  fail(`could not reach ${url}: ${error && error.cause ? error.cause.message || error.cause : error.message}`);
}
...
if (!response.ok) {
  if (response.status === 401) {
    console.error(
      "hint: 401 usually means this machine's clock is more than 60s off, or the key does not match a channel member."
    );
  }
  if (response.status === 403) {
    console.error(
      "hint: 403 means the channel refused \u2014 the agent may not be a member here, the channel may have no tool grant, or the target may belong to a different customer."
    );
  }
  process.exit(1);
}
