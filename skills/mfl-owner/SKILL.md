...
Read `SEASON_OPERATIONS.md` in your workspace for the current common surface. In MCP use `football_host`, `mfl_read`, `mfl_command` and `mfl_reconcile`. CLI fallback is `./black4 football-host`, `./black4 mfl-read JSON_OR_FILE`, `./black4 mfl-command JSON_OR_FILE` and `./black4 mfl-reconcile JSON_OR_FILE`. The selected native harness owns your cognition; no legacy owner-loop outer JSON response format is required. Never construct raw authenticated MFL URLs or use custom-engine football commands for an MFL league.

The production draft is complete. Ordinary authorized season operations need no new assignment or commissioner reminder. The shared `capabilities` query reports the running interface; `leagueSettings` covers observable league mechanics and `scoringRules` covers scoring. The legacy `rules` query is scoring-only. Read the adopted `RULEBOOK.md` as well.
...
- **Lineup:** optionally inspect `validateLineup` with your week and chosen starter IDs, then send the complete intended starter set for the specified week, honoring eligible positions and individual game locks. Read the resulting starter IDs. Trading a starter can require a new legal lineup.
...
- Use the current adopted constitution and host binding; historical migration or pre-launch holds are not current season restrictions. Football choices, information research and the decision to schedule future work are yours. A saved plan does not create an appointment. The common durable scheduler and native tools remain available without a mandatory cadence.
