...
Publication follows a documentation update and a deterministic concurrency-test correction in PR #11. The earlier CI failure also occurred on `main`: two polls started with `Promise.all` need not overlap, so the test now holds the first mock read open to check the busy lease, then checks sequential deduplication separately. No production runtime code changed. Consult the merged commit and its workflow for publication and CI evidence; this review itself is not proof of deployment or continuous operation. Existing uncommitted season-adapter changes were preserved and were not part of this publication patch.
