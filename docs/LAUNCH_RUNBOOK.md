...
NFL.com was rechecked September 7: the opener is September 9 at 8:20 PM Eastern / 5:20 PM Pacific. Public launch target: 4:20 PM Pacific. Draft target: 10 AM Pacific. Source: https://www.nfl.com/schedules/2026/by-week/week-1.

The local foundation is an early build. Launch requires actual owners, authenticated model and data calls, remote persistence, human controls and a deployed public site. Synthetic tests cannot substitute for those observations.

## Critical path

1. **Access:** dedicated model accounts/keys and live NFL feed; exact ten candidate tool/cost canaries; second human identity; Buzz operator provisioning. Preserve failed responses as evidence and surface signup/payment dependencies.
2. **Remote reliability:** league-only service and PostgreSQL, isolated agent workspaces, process supervisor, durable volumes, backups, time synchronization, logs and alert destination. Closing Joey's laptop must not stop it. Run an actual remote process-kill/restart test.
...
- Deployed Buzz cross-human DM permission and durable commissioner transcript archive.
- Provider-backed injury/news feeds, source relevance/coalescing and rate budgets.
- Full playoffs/tiebreakers/season close, trade deadlines, dropped-player waiver holds, IR and unusual postponement handling before those features are promised in the constitution.
- Public franchise publishing, batch approval receipts, attribution, first-party growth scoring and qualified-lead delivery.
- Production secret management, HTTPS and least-privilege database/runtime deployment. Local bearer tokens are a rehearsal credential mechanism.
